...
class Evaluate_Functions {
	...
	private function evaluate_snippet_flat_file( array $snippet, string $file_path, ?array $edit_snippet = null ) {
		...
		$pages = isset( $snippet['pages'] ) ? $snippet['pages'] : '';
		...
		if ( ! empty( trim( $pages ) ) ) {
			if ( is_admin() ) {
				if ( code_snippets_match_page_restrictions_admin( $pages ) ) {
			...
			if ( code_snippets_match_page_restrictions_early( $pages ) ) {
				if ( apply_filters( 'code_snippets/allow_execute_snippet', true, $snippet_id, $table_name ) ) {
					execute_snippet_from_flat_file( $code, $file_path, $snippet_id );
				}
			} else {
				add_action( 'wp', function() use ( $code, $file_path, $snippet_id, $table_name, $pages ) {
					if ( code_snippets_match_page_restrictions_late( $pages ) ) {
						if ( apply_filters( 'code_snippets/allow_execute_snippet', true, $snippet_id, $table_name ) ) {
							execute_snippet_from_flat_file( $code, $file_path, $snippet_id );
						}
					}
		...
		foreach ( $active_snippets as $snippet ) {
			...
			if ( ! empty( trim( $pages ) ) ) {
				...
				if ( code_snippets_match_page_restrictions_early( $pages ) ) {
					...
					add_action( 'wp', function() use ( $code, $snippet_id, $table_name, $edit_snippet, $pages ) {
						if ( code_snippets_match_page_restrictions_late( $pages ) ) {
							if ( apply_filters( 'code_snippets/allow_execute_snippet', true, $snippet_id, $table_name ) &&
							     ( is_null( $edit_snippet ) || $edit_snippet['id'] !== $snippet_id || $edit_snippet['table'] !== $table_name ) ) {
								execute_snippet( $code, $snippet_id );
